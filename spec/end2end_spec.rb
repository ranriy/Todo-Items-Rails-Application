...
feature "Module 4 End-2-End Test" do
    ...
    context "rq14" do
        ...
        let (:userLists) { user.authenticate("123abc").todolists }
        ...
        scenario "End to end works from login through TodoList and TodoItem operations" do
            ...
            numItems = curList.todoitems.count
            ...
            expect(page).to have_button("Create Todoitem")

            fill_in 'todoitem[title]', with: item.title                # load form
            ...
            expect(curList.todoitems.count).to eq(numItems + 1)
            ...
            expect(curList.todoitems.count).to eq(numItems)
            # confirm that we are back on summary TodoList item view
            #puts "******", curList.list_name
            expect(page).to have_content(curList.list_name) 
            ...
            expect(page).to have_button("Create Todolist")

            list = Todolist.new(list_name:'New List', list_due_date:Date.today)
            select list.list_due_date.year, from: 'todolist[list_due_date(1i)]'
            select list.list_due_date.strftime("%B"), from: 'todolist[list_due_date(2i)]'
            select list.list_due_date.day, from: 'todolist[list_due_date(3i)]'
            fill_in 'todolist[list_name]', with: list.list_name
            ...
            expect(user.authenticate("123abc").todolists.count).to eq(totalLists + 1)
            ...
            expect(user.authenticate("123abc").todolists.count).to eq(totalLists)
